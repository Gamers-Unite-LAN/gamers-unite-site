<template>
  <main class="mx-auto w-full max-w-6xl px-4 py-16 sm:px-6 lg:px-8">
    <header class="mx-auto max-w-3xl text-center">
      ...
      <p class="mt-5 text-lg leading-8 text-muted-foreground">Pick one game in each category for the next Gamers Unite!
        LAN. Sign in with Discord so every person gets one vote per poll.</p>
    </header>

    <p v-if="loading" class="py-16 text-center text-muted-foreground" role="status">Loading polls…</p>
    <p v-else-if="error"
      class="mx-auto mt-10 max-w-2xl rounded-xl border border-destructive/30 bg-destructive/10 p-4 text-center text-destructive"
      role="alert">{{ error }}</p>
    ...
    <template v-else>
      <section
        class="mx-auto mt-10 flex max-w-5xl flex-wrap items-end justify-between gap-4 rounded-2xl border bg-card p-6 shadow-sm"
        ...
        <div>
          ...
          <p v-if="pollStatus === 'open'" class="mt-2 text-sm text-muted-foreground">Closes {{
            dateTimeLabel(pollSchedule.closeAt) }}</p>
          <p v-else-if="pollStatus === 'scheduled'" class="mt-2 text-sm text-muted-foreground">Opens {{
            dateTimeLabel(pollSchedule.openAt) }}</p>
      ...
      <div class="mt-8 grid gap-6 lg:grid-cols-3">
        <article v-for="poll in orderedPolls" :key="poll.category"
          class="overflow-hidden rounded-2xl border bg-card shadow-sm">
          <header class="border-b bg-muted/20 p-5">
            <div class="flex items-center justify-between gap-3">
              <img :src="categoryImages[poll.category]" :alt="`${poll.category} games`"
                class="h-14 w-full object-contain object-left" />
              <span class="rounded-full border px-2.5 py-1 text-xs font-bold capitalize text-muted-foreground">{{
                poll.status }}</span>
            </div>
            <p v-if="poll.status !== 'open' && poll.status !== 'scheduled'" class="mt-2 text-sm text-muted-foreground">
              Final results</p>
          </header>
          ...
          <div class="space-y-3 p-5">
            <button v-for="(result, index) in poll.results" :key="result.gameName" type="button"
              ...
              <div class="flex items-center justify-between gap-3">
                ...
                <span class="shrink-0 text-sm font-bold text-muted-foreground">{{ result.votes }} vote{{ result.votes
                  === 1 ? '' : 's' }}</span>
              </div>
              <div class="mt-3 h-2 overflow-hidden rounded-full bg-muted" aria-hidden="true">
                <div class="h-full rounded-full bg-primary transition-all"
                  :style="{ width: `${Math.max(result.votes, result.winner ? 1 : 0) / Math.max(1, Math.max(...poll.results.map((candidate) => candidate.votes))) * 100}%` }">
                </div>
              </div>
              <span v-if="poll.voterGameIndex === index" class="mt-2 block text-xs font-bold text-primary">Your
                vote</span>
              <span v-if="poll.status === 'closed' && result.winner"
...
const categoryImages: Record<Poll["category"], string> = {
  ...
  classic: "/game-poll-categories/classic.png",
  wildcard: "/game-poll-categories/wildcard.png",
};
